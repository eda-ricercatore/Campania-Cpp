...
StudentDB::StudentDB() {
	...
	for(int i=0; i<MAXIMUM_NUMBER_OF_STUDENTS; i++) {
		student_records[i].set_lastname("Lastname");
		student_records[i].set_firstname("Firstname");
		/**
		 ...
		 */
		student_records[i].set_ssn("999-99-9999");
		student_records[i].set_phone("(999)999-9999");
		student_records[i].set_age(120);
	}
}
 ...
 */
bool StudentDB::read_input(string inputfilename) {
	...
	while((inputfile != NULL) && (i<MAXIMUM_NUMBER_OF_STUDENTS)) {
		...
		add_single_student(tempStudent);
	}
 ...
 */
void StudentDB::write_output(string outputfilename) {
	...
	for(int i=0; i<number_correct_records; i++) {
		outputfile.width(16);
		outputfile << student_records[i].get_lastname();
		outputfile.width(16);
		outputfile << student_records[i].get_firstname();
		outputfile.width(16);
		outputfile << student_records[i].get_ssn();
		outputfile.width(16);
		outputfile << student_records[i].get_phone();
		outputfile.width(16);
		outputfile << student_records[i].get_age();
		outputfile << endl;
	}
	outputfile << endl;
...
bool StudentDB::check_duplicate_social_security_number(string ssn,
	...
	for(int i=0; i<MAXIMUM_NUMBER_OF_STUDENTS; i++) {
		if(i==current_index) {
			continue;
		}else if(student_records[i].get_ssn() == ssn) {
			return false;
		}
	}
	return true;
 ...
 */
void StudentDB::display_single_student_details(int index) {
	cout.setf(ios::left);
	cout.width(MAXIMUM_NUMBER_OF_CHARACTERS);
	cout << student_records[index].get_lastname();
	cout.width(MAXIMUM_NUMBER_OF_CHARACTERS);
	cout << student_records[index].get_firstname();
	cout.width(MAXIMUM_NUMBER_OF_CHARACTERS);
	cout << student_records[index].get_ssn();
	cout.width(MAXIMUM_NUMBER_OF_CHARACTERS);
	cout << student_records[index].get_phone();
	cout.width(MAXIMUM_NUMBER_OF_CHARACTERS);
	cout << student_records[index].get_age() << endl;
}
 ...
 * database.
 */
void StudentDB::add_single_student(Student student_details) {
	// Value of the binary search results
	int result = binary_search(student_details.get_ssn(),
	...
	if(result != (-1)) {
		...
		cout << "A student with the social security number, "
			<< student_records[result].get_ssn()
			<< ", already exists in the database" << endl;
	}else{
		// Is the database full?
		if(get_index_of_last_element() >= (MAXIMUM_NUMBER_OF_STUDENTS-1)) {
			...
			student_records[get_index_of_last_element()+1].set_lastname(
				student_details.get_lastname());
			student_records[get_index_of_last_element()+1].set_firstname(
				student_details.get_firstname());
			student_records[get_index_of_last_element()+1].set_ssn(
				student_details.get_ssn());
			student_records[get_index_of_last_element()+1].set_phone(
				student_details.get_phone());
			student_records[get_index_of_last_element()+1].set_age(
 ...
 */
void StudentDB::remove_selected_student(string ssn) {
	if(!check_socialsecuritynumber(ssn)) {
		cout << "Social security number entered is invalid" << endl;
	}else{
		// Value of the binary search results
		int result = binary_search(ssn, 0,(MAXIMUM_NUMBER_OF_STUDENTS-1));
		/**
		 * If a student with the same social security number as this person's
 ...
 */
void StudentDB::display_all_student_details() {
	// Print the header...
	print_header();
	// Print the particulars...
	for(int i=0; i<get_number_correct_records(); i++) {
		display_single_student_details(i);
	}
}
